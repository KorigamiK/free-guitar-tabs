...
"""
Run this first ->
youtube-dl --skip-download --write-description https://www.youtube.com/channel/UCjN_KqUzrMTfnr03fzk_AAQ/videos
"""
    ...
    async with aiofiles.open(file_name, mode="r") as f:
        ...
        async for line in f:
            if 'http' in line and backlist_pattern.search(line) is None:
                print(f"{identity}. [{name}]({link_pattern.search(line.strip()).group(0).strip()})")
                flag = False
        if flag:
